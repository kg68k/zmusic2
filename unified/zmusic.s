...
.ifdef TINY	*TINY版 zmsc.x
tr_max:		equ	32		*確保出来るトラック数の最大値
dflt_trkbf:	equ	32*1024		*デフォルトトラックバッファサイズ
DFLT_TRKBF_STR:	reg	'32k'
at_least_wk:	equ	0		*ワークエリアのミニマムサイズ(通常)
AT_LEAST_WK_STR:reg	'0k'
*at_least_wk2:	equ	0 		*ワークエリアのミニマムサイズ(COMPILE時)
.else		*FULL版 zmusic.x
tr_max:		equ	80		*確保出来るトラック数の最大値
dflt_trkbf:	equ	128*1024	*デフォルトトラックバッファサイズ
DFLT_TRKBF_STR:	reg	'128k'
at_least_wk:	equ	 12*1024	*ワークエリアのミニマムサイズ(通常)
AT_LEAST_WK_STR:reg	'12k'
at_least_wk2:	equ	128*1024	*ワークエリアのミニマムサイズ(COMPILE時)
.endif

dflt_pcmbf:	equ	256*1024	*デフォルトＰＣＭバッファサイズ
DFLT_PCMBF_STR:	reg	'256k'
...
@@:
	...
	.include	skipspc.s	*skip_spc: skip_plus: skip_sep:
...
exit_mkcptl:
	...
	IF_TINY	<.include skipspc.s>	*skip_spc: skip_plus: skip_sep:
...
exit_adpcm_cnf:			*case error
	...
	.include	getfdate.s	*get_filedate:
	.include	fileio.s	*so_read: so_ope:
...
exit_sen:
	...
	IF_TINY	<.include copyfn.s>	*copy_fn:
...
dev_inp:			*MIDI生データ入力
	...
	ZMCALL	#$17		*QUIT FROM MIDI DATA RECORDING
dvi_lp01:
...
com_max:	equ	(m_job_tbl_end-m_job_tbl)/2-1	*MAXコマンドナンバー
		...
		IF_TINY	<dc.b 'TINY'>
		dc.b	'ZmuSiC'	*ID
ver_num:	dc.w	VERSION_CODE	*ZMUSIC VERSION NUMBER
...
case_c_i0:
	IF_FULL	<move.l #$0000_00_ff,adpcm_bank-work(a6)>	*adpcm_bank(.w),err_exist_f(.b),fm_master(.b)
...
case_c_i1:				*cp!!(コンパイルモード時の初期化はここまで)
	...
	IF_FULL	<bsr init_trk_len>	*各トラックバッファの長さを初期化
	bsr	trk_top_set_
	IF_FULL	<bsr tr_top_set>	*コンパイルデータ格納ポインタを先頭へ
	bsr	tr_end_set		*dummyエンドコードセット
	bsr	top_ptr_set_all		*演奏ポインタを全て先頭へ
	bsr	init_play_wk		*演奏ワークの初期化
	bsr	init_cmn_wks		*共通ワーク初期化
	move.w	(sp)+,sr
	IF_FULL	<bsr init_cnv_wk>	*コンバートワークの初期化
	bsr	init_midi		*MIDI楽器の初期化
	moveq.l	#0,d2			*(b0)
	bsr	set_ch_mode
	IF_FULL	<bsr relative_uv>	*(u0)
	bra	t_dat_ok
...
adfop0:
	...
	IF_FULL	<clr.b last_fn-work(a6)>	*!!!2.03
...
get_cm_l:
	...
	IF_TINY	<.include fileio.s>	*so_read: so_ope:
	IF_TINY	<.include getfdate.s>	*get_filedate:
...
set_loop_time:			*ループ回数とジャンプ先の設定
	...
	* < d2.l=loop time(s)
	...
	move.l	d0,(a0)+	*0-31
	IF_FULL	<move.l d0,(a0)+>	*32-63
	IF_FULL	<move.w d0,(a0)+>	*64-79
	rts			*d0=0
...
set_fm_master_vol:		*ＦＭマスターボリュームを設定する
	...
	* < d2.b=master voluem value(0-255)
	IF_FULL	<move.b d2,fm_master-work(a6)>
	bra	t_dat_ok
...
neiro_init:			*音色バッファの初期化
	FT_LEA	neiro_,a1			*src
	IF_FULL	<movea.l neiro-work(a6),a2>	*dest
	IF_TINY	<movea.l neiro(pc),a2>		*dest
	moveq.l	#5,d1		*mode
...
@@:
	...
	dbra	d1,@b
	move.b	d0,(a0)
	IF_FULL	<move.b d0,err_exist_f-work(a6)>
	movem.l	(sp)+,d1/a0
	rts

	IF_FULL	<.include mml_compile.s>
...
calc_wk:
	* < d0=trk number(full:0-79, tiny:0-31)
	* > a5=work addr.
	move.l	d0,-(sp)
	lsl.w	#wk_size2,d0
	movea.l	seq_wk_tbl(pc),a5
...
dev_init:				*デバイスドライバとしての初期化
	movem.l	d1-d7/a0-a6,-(sp)
	lea	work(pc),a6
	IF_FULL	<lea -390(sp),sp>
	move.l	sp,_sp_buf-work(a6)

	IF_TINY	<movea.l 18(a5),a4>	*parameter pointer
	lea	-260(sp),sp
	move.l	sp,dmy_seq_wk-work(a6)
	IF_FULL	<bsr gj_copy>
	IF_FULL	<movea.l 18(a5),a4>	*parameter pointer
di_lp01:
	tst.b	(a4)+
...
@@:
	...
	bsr	read_start_up
@@:
	IF_FULL <lea 260+390(sp),sp>
	IF_TINY	<lea 260(sp),sp>
	movem.l	(sp)+,d1-d7/a0-a6
	bra	ok_com
...
exec:				*コマンドラインから実行した時
	...
	IF_FULL	<lea -390(sp),sp>
	IF_FULL	<move.l sp,_sp_buf-work(a6)>
...
@@:
	bsr	init_midibd
	moveq.l	#0,d0
	IF_FULL	<move.b d0,compile_mode-work(a6)>	*compile mode off
...
no_more?:
	...
	bne	set_default_trkbuf	*コマンドラインからならもうスイッチはあり得ない
	tst.b	(a4)
	beq	set_default_trkbuf	*ゼロが２個続くと言うことはもうスイッチは無し
	bra	chk_dev_lp
...
.ifndef TINY	*ここからFULL版のみ
	cmpi.b	#'C',d0		*最適化の有無(大文字のCなら最適化なし)
...
open_and_size:
	...
	* X d0,d3,d5
	FT_JBSR	fopen		*<a2.l=file name,(ret:d5=file handle)
	tst.l	d5		*d5=file_handle
	bmi	oas_err
	FT_JBSR	get_fsize2	*>d3.l=file size
	bmi	oas_err
	cmp.l	(a1),d3
...
@@:
	...
	IF_TINY	<.include chknum.s>	*chk_num:
...
@@:
	...
	FT_LEA	Z_MUSIC_t3,a1
	IOCS	_B_INTVCS
	move.l	d0,sv_trap3-Z_MUSIC_t3(a1)

	move.l	int_rte(pc),rte_src-work(a6)
	IF_FULL	<move.l m_play00-work(a6),m_play00_bak-work(a6)>
	IF_TINY	<move.l m_play00(pc),m_play00_bak-work(a6)>
prt_title:				*タイトル表示
	tst.b	cmd_or_dev-work(a6)
	bne	@f
	lea.l	crlf(pc),a0
...
rewrt_ff:			*$ff50～$ff7fを$ff80～$ffafに移動する
	...
	lea	rw_tbl(pc),a0
@@:
	IF_FULL	<move.l (a0)+,d0>
	IF_TINY	<move.w (a0)+,d0>
	beq	exit_rwff
	IF_FULL	<lea rw_tbl(pc,d0.l),a1>
	IF_TINY	<lea rw_tbl(pc,d0.w),a1>
	add.w	#$30,(a1)
	bra	@b
exit_rwff:
	movem.l	(sp)+,d0/a0-a1
...
nmdb_tbl:
  ...
  IF_FULL <dc.w mdbd_chk-work,t_err_6-mdbd_chk-2>
...
mdb_tbl:
  IF_FULL <dc.w mdb0-work,mmlnn-mdb0-2>
	dc.w	0
...
npcm8_tbl:	*not pcm8 mode
  IF_FULL <dc.w set_exch?-work,pex0-set_exch?-2>
...
trk_len_tbl__:	equ	0
.else
trk_len_tbl__:	equ	tr_max*4
.endif
trk_po_tbl__:	equ	tr_max*4
seq_wk_tbl__:	equ	wk_size*tr_max
seq_wk_tbl2__:	equ	wk_size*32	*pl_max
.ifdef TINY
cnv_wk_tbl__:	equ	0
.else
cnv_wk_tbl__:	equ	cnv_wk_size*tr_max
.endif
neiro__:	equ	neiro_size*tone_max
uwv__:		equ	wv_max*14
...
get_work_area:			*ワークエリアの確保
	...
	IF_FULL	<move.l a1,trk_len_tbl-work(a6)>
	IF_FULL	<lea trk_len_tbl__(a1),a1>
	...
	IF_FULL	<move.l a1,cnv_wk_tbl-work(a6)>
	IF_FULL	<lea cnv_wk_tbl__(a1),a1>
...
back_copy_key:			*COPY KEY VECTOR復元
	...
	move.l	a0work(pc),a0
@@:
	move.l	(a0),d0		*前のメモリ管理ポインタ
	beq	@f
	move.l	d0,a0		*先頭のメモリブロック(Human68k)まで遡る
	bra	@b
@@:
	lea	copy_org(pc),a1
	move.l	(a1,d7.l),d1
...
bck_lp:
	...
	beq	exit_bck
	andi.l	#$ff_ffff,d0	*アドレスの常駐フラグをクリア
	move.l	d0,a2

	move.l	a0,a3		*先頭のメモリブロック
@@:
	...
	bra	@b
@@:
*a2が常駐部のZMSC.Xの親プロセスやその祖先である保証はない。
*(親プロセスが終了後に新たにロードされたプロセスや、
*通常のメモリブロックの可能性がある)
	move.l	d1,$54(a2)	*PSP内に保存されたtrap #12ベクタ
	bra	bck_lp
...
bcklp:
* a0が常駐プロセスか確認していない。
* $54(a0)がメモリブロック内を指すか確認していない。
* これによりメモリ内容を破壊したり、バスエラーが発生する。
	cmp.l	$54(a0),a2	*PSP内に保存されたtrap #12ベクタ
	bne	@f
	move.l	d1,$54(a0)
@@:
...
keep_mes:
	dc.b	COL3_EM
	VER_TYPE_STR <SUBSCRIPT_S>
	dc.b	' '
	SUBSCRIPT_S 'VERSION'
	dc.b	' '
	.irp	ch,VERSION_MAJOR+'0','.',VERSION_MINOR1+'0',VERSION_MINOR2+'0'
		SUBSCRIPT_C ch
	.endm
	dc.b	COL_DEF,' (C) '
	IF_FULL	<dc.b '1991,'>
	dc.b	'1992,1993,1994 '
	dc.b	COL2_EM,'ZENJI SOFT',COL_DEF,CRLF,0
yes_midi:	dc.b	"MIDI/"
no_midi:	dc.b	"FM･OPM/ADPCM are under the control of ZMUSIC.",CRLF,0

.ifdef RSMIDI
already_mes:	dc.b	COL3_EM_REV,'The interrupt-vector has already been occupied by some other applications.',COL_DEF,CRLF,0
.else
already_mes:	dc.b	COL3_EM_REV,'MUSIC BIOS has already been included in your system.',COL_DEF,CRLF,0
.endif
already_mes2:	dc.b	COL3_EM_REV,"Didn't you include a special ADPCM driver?",COL_DEF,CRLF,0

.ifndef TINY
out_mem_mes:	dc.b	COL3_EM_REV,'Out of memory.',COL_DEF,CRLF,0
.endif

secure_mes:	dc.b	'Secure ',0
pcmbf_mes:	dc.b	'for ADPCM data buffer.',CRLF,0
wkbf_mes:	dc.b	'for work area.',CRLF,0
trkbf_mes:	dc.b	'for track buffer.',CRLF,0
kakuho_mes:	dc.b	'kByte(s)',COL_DEF,' ',0
atrb_0010:	dc.b	COL2,0
tm_a_mes:	dc.b	'Interrupt source:TIMER A',CRLF,0
sq_m_mes:	dc.b	'The external MIDI sequencer synchronizes with ',EXEFILE,CRLF,0

.ifdef POLYPHON
pcm8_mode_mes_:	dc.b	'MIDI and ADPCM are managed by PCM8SB.X (C)H.ETOH',CRLF,0
.endif
pcm8_mode_mes:	dc.b	'ADPCM is managed by PCM8.X (C)H.ETOH',CRLF,0

zmt:	
	IF_FULL	<dc.b COL2,$eb,$ee,COL1,'-',$eb,$ef,$eb,$f0,COL_DEF,$eb,$f1,COL1,$eb,$f2,$eb,$f3>
	IF_TINY	<dc.b COL2_EM,'Ζ',COL1_EM,'m',COL3_EM,'∫',COL1_EM,'c '>
		dc.b	0

kaijo:		dc.b	COL_DEF,'has been released from your system.',CRLF,0
not_kep_mes:	dc.b	COL_DEF,'is not kept in your system.',CRLF,0
rls_er_mes:	dc.b	COL_DEF,'is unable to release.',CRLF,0
ver_er_mes:	dc.b	COL3_EM_REV,'Illegal version number. Unable to release.',COL_DEF,CRLF,0
illegal_p:	dc.b	COL3_EM_REV,'ERROR',COL_DEF,CRLF,0
unknown_mes:	dc.b	COL3_EM_REV,'Unknown error!'
		dc.b	COL_DEF,CRLF,0
err_header:	dc.b	COL3_EM_REV,0

	IF_FULL	<.include mml_mes.s>

sr_filename:		*メモリの有効利用?
help_mes:	dc.b	COL3_EM,'< USAGE >'
		dc.b	COL_DEF,' ',EXEFILE,' [Optional Switches]'
			IF_FULL	<dc.b ' [-C･-Q ',$3c,'ZMS filename',$3e,' [ZMD filename]]'>
			dc.b	CRLF
		dc.b	COL3_EM,'< OPTIONAL SWITCHES >',CRLF,COL_DEF
		dc.b	'-? or H      Display the list of optional switches.',CRLF
		dc.b	'-A	     Make ',EXEFILE,' work on TIMER-A.',CRLF
		dc.b	'-B<filename> Include ADPCM block data.',CRLF
	IF_FULL	<dc.b	'-C	     Convert ZMS file into ZMD file.',CRLF>
sv_filename:
.ifdef CZ6BMI
		dc.b	'-E	     Synchronize an external MIDI sequencer.',CRLF
.endif
		dc.b	'-P<n>	     Secure n kBytes for ADPCM data buffer.(default=',DFLT_PCMBF_STR,'B)',CRLF
	IF_FULL	<dc.b	'-Q	     Convert ZMS file into ZMD file and calculate the total step count.',CRLF>
		dc.b	'-R	     Release ',EXEFILE,' from the system.',CRLF
		dc.b	'-S<filename> Include start-up file.',CRLF
		dc.b	'-T<n>	     Secure n kBytes for the MML track buffer.(default=',DFLT_TRKBF_STR,'B)',CRLF
		dc.b	'-W<n>	     Secure n kBytes for a work area.(default=',AT_LEAST_WK_STR,'B)',CRLF
.ifndef TINY	*ここからFULL版のみ
		dc.b	COL3_EM,'< NOTICE >',CRLF
		dc.b	"'-C' or '-Q' must be set in the last.",CRLF
		dc.b	COL_DEF
.endif		*ここまでFULL版のみ
		dc.b	0
